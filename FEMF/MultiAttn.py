...
#         super().__init__()
#         self.attn_text_audio = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=num_heads, batch_first=False)
#         self.attn_text_visual = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=num_heads, batch_first=False)
...
#         self.fusion_proj = nn.Linear(hidden_dim * 2, input_dim)  # 返回原维度匹配 MultiAttn
#         self.norm = nn.LayerNorm(input_dim)

#     def forward(self, text, audio, visual):
#         """
#         text, audio, visual: [T, B, D]
#         return: fused_text: [T, B, D]
#         """
#         Q = self.text_proj(text)      # [T, B, H]
#         K_a = self.audio_proj(audio)
#         V_a = self.audio_proj(audio)

#         K_v = self.visual_proj(visual)
#         V_v = self.visual_proj(visual)

#         # Cross-attention: text attends to audio & visual
#         text_audio_out, _ = self.attn_text_audio(Q, K_a, V_a)
#         text_visual_out, _ = self.attn_text_visual(Q, K_v, V_v)

#         # 拼接两个方向的融合表示
#         fusion = torch.cat([text_audio_out, text_visual_out], dim=-1)  # [T, B, 2H]

#         fused = self.fusion_proj(fusion)  # [T, B, D]
#         fused = self.norm(text + fused)   # residual 保留原文本表示
...
class MultiAttnModel(nn.Module):
    ...
    def forward(self, text_features, audio_features, visual_features):
        ...
        # f_t = self.multiattn_text(text_features, visual_features, audio_features)
        f_a = self.multiattn_audio(audio_features, text_features, visual_features)
        # f_a = self.multiattn_audio(audio_features, visual_features, text_features)
        f_v = self.multiattn_visual(visual_features, text_features, audio_features)
        # f_v = self.multiattn_visual(visual_features, audio_features, text_features)
