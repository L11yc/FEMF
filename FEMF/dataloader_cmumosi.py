...
def read_data(label_path, feature_root):
    ...
    for ii, name in enumerate(names):
        ...
        if os.path.exists(feature_path):
            single_feature = np.load(feature_path, allow_pickle=True)
...
class CMUMOSIDataset(Dataset):
    ...
    def __init__(self, label_path, audio_root, text_root, video_root):
        ...
        def pad_to_1024(feat_list):
                padded_feats = []
                for i, feat in enumerate(feat_list):
                    if feat.shape[0] < 1024:
                        print(f"[Padding] Frame {i} has shape {feat.shape}, padding to (1024,)")
                        feat = np.pad(feat, (0, 1024 - feat.shape[0]), mode='constant')
                    elif feat.shape[0] > 1024:
                        print(f"[Truncate] Frame {i} has shape {feat.shape}, truncating to (1024,)")
                        feat = feat[:1024]
                    padded_feats.append(feat)
                return np.array(padded_feats)
        for ii, vid in enumerate(sorted(self.videoIDs)):
            ...
            self.videoVisualHost[vid] = pad_to_1024(self.videoVisualHost[vid])
